...
namespace PACGRAPH_2018 {
	public class CP_Resad : MonoBehaviour {
		...
		void Update() {
			...
			if(NN.Model.Parameters != null) {
				...
				((PFNN_res_cp)NN.Model).Predict_res(InStyle);
				...
				//Update Phase
				((PFNN_res_cp)NN.Model).SetPhase(Mathf.Repeat(((PFNN_res_cp)NN.Model).GetPhase() + (rest * 0.9f + 0.1f) * NN.Model.GetOutput(3) * 2f*Mathf.PI, 2f*Mathf.PI));
			}
		}
		...
		void OnRenderObject() {
			...
			Quaternion rotation = Quaternion.AngleAxis(-360f * ((PFNN_res_cp)NN.Model).GetPhase() / (2f * Mathf.PI), Vector3.forward);
	...
	#if UNITY_EDITOR
	[CustomEditor(typeof(CP_Resad))]
	public class CP_Resad_Editor : Editor {

			public CP_Resad Target;

			void Awake() {
				Target = (CP_Resad)target;
			}
