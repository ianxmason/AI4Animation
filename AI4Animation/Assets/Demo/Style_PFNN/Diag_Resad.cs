...
namespace PACGRAPH_2018 {
	public class Diag_Resad : MonoBehaviour {
		...
		void Update() {
			...
			if(NN.Model.Parameters != null) {
				...
				((PFNN_res_diag)NN.Model).Predict_res(InStyle);
				...
				//Update Phase
				((PFNN_res_diag)NN.Model).SetPhase(Mathf.Repeat(((PFNN_res_diag)NN.Model).GetPhase() + (rest * 0.9f + 0.1f) * NN.Model.GetOutput(3) * 2f*Mathf.PI, 2f*Mathf.PI));
			}
		}
		...
		void OnRenderObject() {
			...
			Quaternion rotation = Quaternion.AngleAxis(-360f * ((PFNN_res_diag)NN.Model).GetPhase() / (2f * Mathf.PI), Vector3.forward);
	...
	#if UNITY_EDITOR
	[CustomEditor(typeof(Diag_Resad))]
	public class Diag_Resad_Editor : Editor {

			public Diag_Resad Target;

			void Awake() {
				Target = (Diag_Resad)target;
			}
