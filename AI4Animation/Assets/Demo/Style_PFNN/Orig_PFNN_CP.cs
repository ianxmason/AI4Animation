...
namespace SIGGRAPH_2017 {
	public class Orig_PFNN_CP : MonoBehaviour {
		...
		private float UnitScale = 2f; // 100f;
		...
		void Update() {
			...
			*/
			...
			if(NN.Model.Parameters != null) {
				...
				//Update Phase
				((PFNN_CP)NN.Model).SetPhase(Mathf.Repeat(((PFNN_CP)NN.Model).GetPhase() + (rest * 0.9f + 0.1f) * NN.Model.GetOutput(3) * 2f*Mathf.PI, 2f*Mathf.PI));
			}
		}
		...
		void OnRenderObject() {
			...
			Quaternion rotation = Quaternion.AngleAxis(-360f * ((PFNN_CP)NN.Model).GetPhase() / (2f * Mathf.PI), Vector3.forward);
	...
	#if UNITY_EDITOR
	[CustomEditor(typeof(Orig_PFNN_CP))]
	public class Orig_PFNN_CP_Editor : Editor {

			public Orig_PFNN_CP Target;

			void Awake() {
				Target = (Orig_PFNN_CP)target;
			}
