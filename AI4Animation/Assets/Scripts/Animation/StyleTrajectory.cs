...
public class StyleTrajectory {
	...
	public StyleTrajectory(int size, int gaits, int styles) {
		Inspect = false;
		Points = new Point[size];
		for(int i=0; i<Points.Length; i++) {
			Points[i] = new Point(i, gaits, styles);
			Points[i].SetTransformation(Matrix4x4.identity);
		}
	}

	public StyleTrajectory(int size, int gaits, int styles, Vector3 seedPosition, Vector3 seedDirection) {
		Inspect = false;
		Points = new Point[size];
		for(int i=0; i<Points.Length; i++) {
			Points[i] = new Point(i, gaits, styles);
	...
	public StyleTrajectory(int size, int gaits, int styles, Vector3[] positions, Vector3[] directions) {
		Inspect = false;
		Points = new Point[size];
		for(int i=0; i<Points.Length; i++) {
			Points[i] = new Point(i, gaits, styles);
	...
	public class Point {
		...
		public float[] Gaits = new float[0];
        public float[] Styles = new float[0];

		public Point(int index, int gaits, int styles) {
			Index = index;
		...
		public void Postprocess() {
			...
			RightSample = position + StyleTrajectory.Width * ortho.normalized;
			RightSample.y = Utility.GetHeight(RightSample, mask);
			LeftSample = position - StyleTrajectory.Width * ortho.normalized;
