...
namespace DeepLearning {
	...
	public class PFNN_res_diag : Model {
		...
		private Tensor[] W0, W1, W2, b0, b1, b2, temp_tens;
		...
		public void Predict_res(string style) {
			...
			for(int i=0; i<switch_list.Length; i++){
				if (style == switch_list[i]){
					ELU(Add(Layer(Y, W1[index], b1[index], H1), Add(PointwiseProduct(W[i][index], Y, H2), b[i][index], H2), Y));
				}
				else {
					counter2++;
				}
