...
namespace DeepLearning {
	...
	public class PFNN_res_full : Model {
		...
		private Tensor[] W0, W1, W2, b0, b1, b2, temp_tens, style_W, style_b;
		private List<Tensor[]> W, b;
		private Tensor X, H1, H2, Y;

		private string line; 
		private string[] style_list = new string[65];
		private string[] style_count = new string[65];
		private string[] switch_list = new string[65];

		private float Phase;

		private const float M_PI = 3.14159265358979323846f;

		public PFNN_res_full() {
			int counter = 0;
		...
		public override void StoreParameters() {
			...
			// for(int i=0; i<65; i++){
			for(int i=20; i<26; i++){ // Not enough memory to load them all
				for(int j=0; j<50; j++) {
					Parameters.Store(Folder+style_list[i]+"_W0_"+j.ToString("D3")+".bin", HDim, HDim);
				}
			}	
		...
		public override void LoadParameters() {
			...
			W = new List<Tensor[]>(58);
			for(int i=0; i<58; i++){
				W.Add(new Tensor[50]);
			}
			b = new List<Tensor[]>(58);
			for(int i=0; i<58; i++){
				b.Add(new Tensor[50]);
			}
			...
			for(int i=20; i<26; i++){ 
				for(int j=0; j<50; j++) {
					...
					W[i][j] = CreateTensor(Parameters.Load(4 + 6*50 + (i-20)*50 + j), switch_list[i]+"_W_"+j);
				}
			}	
			...
			for(int i=20; i<26; i++){ 
				for(int j=0; j<50; j++) {
					// b[i][j] = CreateTensor(Parameters.Load(4 + 6*50 + 65*50 + i*50 + j), switch_list[i]+"_b_"+j);
		...
		public void Predict_res(string style) {
			...
			for(int i=0; i<switch_list.Length; i++){
				if (style == switch_list[i]){
                    ELU(Add(Layer(Y, W1[index], b1[index], H1), Layer(Y, W[i][index], b[i][index], H2), Y));
				}
				else {
					counter2++;
				}
