...
def load_to_maya(anim, names=None, radius=0.5, update=False):
    """
    ...
    for i, offset, orient, parent, name in zip(range(len(anim.offsets)), anim.offsets, anim.orients, anim.parents, names):
        ...
        joint = pm.PyNode(name) if update else pm.joint(n=name, p=offset, relative=True, radius=radius)
        ...
        pm.connectAttr(curvex.output, joint.rotateX, force=update)
        pm.connectAttr(curvey.output, joint.rotateY, force=update)
        pm.connectAttr(curvez.output, joint.rotateZ, force=update)
        ...
        pm.connectAttr(offsetx.output, joint.translateX, force=update)
        pm.connectAttr(offsety.output, joint.translateY, force=update)
        pm.connectAttr(offsetz.output, joint.translateZ, force=update)
...
def add_nub(anim, parent, offset, length=1.0):
    
    anim.parents = np.hstack([anim.parents, parent])
    anim.offsets = np.concatenate([anim.offsets, np.array([offset]) * length], axis=0)

    anim.orients.qs = np.concatenate([anim.orients.qs, Quaternions.id((1,)).qs], axis=0)
    
    anim.positions = np.concatenate([
        anim.positions,
        anim.offsets[-1][np.newaxis,np.newaxis].repeat(len(anim.positions), axis=0)], axis=1)
        
    anim.rotations.qs = np.concatenate([
        anim.rotations.qs,
        Quaternions.id((len(anim.rotations), 1)).qs], axis=1)
