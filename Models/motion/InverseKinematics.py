...
class TwoJoint:
    ...
    def __init__(self, anim, hip, knee, heel):
        self.anim = anim
        self.hip = hip
        self.knee = knee
        self.heel = heel
        
    def __call__(self, target, epsilon=0.01):
        
        xforms = Animation.transforms_global(self.anim)
        pos = xforms[:,:,:3,3]
        rot = Quaternions.from_transforms(xforms)
        
        hp2hl = pos[:,self.heel] - pos[:,self.hip]
        hp2tg = target - pos[:,self.hip]
        hp2kn = pos[:,self.knee] - pos[:,self.hip]
        kn2hl = pos[:,self.heel] - pos[:,self.knee]
        
        hp2hl_l = np.sqrt(np.sum(hp2hl**2, axis=-1))
        hp2tg_l = np.sqrt(np.sum(hp2tg**2, axis=-1))
        hp2kn_l = np.sqrt(np.sum(hp2kn**2, axis=-1))
        kn2hl_l = np.sqrt(np.sum(kn2hl**2, axis=-1))
        
        hp2hl_n = hp2hl / hp2hl_l[...,np.newaxis]
        hp2tg_n = hp2tg / hp2tg_l[...,np.newaxis]
        hp2kn_n = hp2kn / hp2kn_l[...,np.newaxis]
        kn2hl_n = kn2hl / kn2hl_l[...,np.newaxis]
        
        """ Extended/Contract """
        
        maxlen = hp2kn_l+kn2hl_l-epsilon
        hp2tg_l[hp2tg_l>maxlen] = maxlen[hp2tg_l>maxlen]
        
        def cosine_rule(a, b, c):
            return np.arccos(np.clip((c*c - a*a - b*b) / (-2*a*b), -1, 1))
        
        hp2hl_hp2kn_a = np.arccos(np.clip(np.sum( hp2hl_n * hp2kn_n, axis=-1), -1, 1))
        kn2hp_kn2hl_a = np.arccos(np.clip(np.sum(-hp2kn_n * kn2hl_n, axis=-1), -1, 1))
        
        hp2hl_hp2kn_b = cosine_rule(hp2kn_l, hp2tg_l, kn2hl_l)
        kn2hp_kn2hl_b = cosine_rule(hp2kn_l, kn2hl_l, hp2tg_l)
        
        axis0 = np.cross(hp2hl, hp2kn)
        axis0 = axis0 / np.sqrt(np.sum(axis0**2, axis=-1))[...,np.newaxis]
        
        axis1 = np.cross(hp2hl, hp2kn)
        axis1 = axis1 / np.sqrt(np.sum(axis1**2, axis=-1))[...,np.newaxis]
        
        rot0 = Quaternions.from_angle_axis(hp2hl_hp2kn_b - hp2hl_hp2kn_a, -rot[:,self.hip] * axis0)
        rot1 = Quaternions.from_angle_axis(kn2hp_kn2hl_b - kn2hp_kn2hl_a, -rot[:,self.knee] * axis1)
        #rot0 = Quaternions.from_angle_axis(np.linspace(0, 1, 100) * (hp2hl_hp2kn_b - hp2hl_hp2kn_a), -rot[:,self.hip] * axis0)
        #rot1 = Quaternions.from_angle_axis(np.linspace(0, 1, 100) * (kn2hp_kn2hl_b - kn2hp_kn2hl_a), -rot[:,self.knee] * axis1)
        
        """ Move """
        
        hp2hl_hp2tg_a = np.arccos(np.clip(np.sum(hp2hl_n * hp2tg_n, axis=-1), -1, 1))
        axis2 = np.cross(hp2hl, hp2tg)
        axis2 = axis2 / np.sqrt(np.sum(axis2**2, axis=-1))[...,np.newaxis]
        rot2 = Quaternions.from_angle_axis(hp2hl_hp2tg_a, -rot[:,self.hip] * axis2)
        #rot2 = Quaternions.from_angle_axis(np.linspace(0, 1, 100) * hp2hl_hp2tg_a, -rot[:,self.hip] * axis2)
                
        """ Done """
        
        self.anim.rotations[:,self.hip]  = self.anim.rotations[:,self.hip] * rot0 * rot2
        self.anim.rotations[:,self.knee] = self.anim.rotations[:,self.knee] * rot1
        
        #self.anim.rotations[:,self.hip]  = self.anim.rotations[:,self.hip] * rot0
        #self.anim.rotations[:,self.knee] = self.anim.rotations[:,self.knee] * rot1
