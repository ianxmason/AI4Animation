...
class AdamTrainer:
    ...
    def __init__(self, rng=np.random, batchsize=16, epochs=100, alpha=0.001, beta1=0.9, beta2=0.999, eps=1e-8, cost='mse', cost2='mse'):
        self.alpha = alpha
        self.beta1 = beta1
        self.beta2 = beta2
        self.eps = eps
        ...
        self.cost2 = cost2
        self.params = None
        self.total_epochs = np.array([0])
        self.train_losses = np.array([0])
    ...
    def get_cost_updates2(self, network, input, output):
        
        cost = self.cost2(network, input, output) + network.cost(input)
        #cost = self.cost(network, input, output)
    ...
    def train(self, network, input_data, output_data, input_data2, output_data2, filename=None, restart=True, shuffle=True, silent=False):
        
        input = input_data.type()
        output = output_data.type()
        index = T.lscalar()
        input2 = input_data2.type()
        output2 = output_data2.type()
        index2 = T.lscalar()
        ...
        cost2, updates2 = self.get_cost_updates2(network, input2, output2)
        train_func2 = theano.function([index2], cost2, updates=updates2, givens={
            input2:input_data2[index2*self.batchsize:(index2+1)*self.batchsize],
            output2:output_data2[index2*self.batchsize:(index2+1)*self.batchsize],
        }, allow_input_downcast=True)
        ...
        for epoch in range(self.epochs):
            ...
            c2 = np.zeros((len(batchinds),))
            for bii, bi in enumerate(batchinds):
                c[bii] = train_func(bi)
                c2[bii] = train_func2(bi)
                if np.isnan(c[bii]): return
                if np.isnan(c2[bii]): return
                if not silent and bii % (int(len(batchinds) / 1000) + 1) == 0:
                    ...
                    sys.stdout.write('\r[Epoch %3i] % 3.1f%% mean %03.5f' % (epoch, 100 * float(bii)/len(batchinds), c2[:bii+1].mean()))
                    sys.stdout.flush()

            curr_mean = c.mean()
            curr_mean2 = c2.mean()
            self.total_epochs = np.append(self.total_epochs, self.total_epochs[-1]+1)
            self.train_losses = np.append(self.train_losses, c.mean())
            diff_mean, last_mean = curr_mean-last_mean, curr_mean
            diff_mean2, last_mean2 = curr_mean2-last_mean2, curr_mean2
            sys.stdout.write('\r[Epoch %3i] 100.0%% mean %03.5f diff % .5f %s' % 
                (epoch, curr_mean, diff_mean, str(datetime.now())[11:19]))
            sys.stdout.write('\r[Epoch %3i] 100.0%% mean %03.5f diff % .5f %s' % 
                (epoch, curr_mean2, diff_mean2, str(datetime.now())[11:19]))
            sys.stdout.flush()
