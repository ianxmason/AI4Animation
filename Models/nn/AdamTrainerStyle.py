...
class AdamTrainer:
    ...
    def __init__(self, rng=np.random, batchsize=16, epochs=100, alpha=0.001, beta1=0.9, beta2=0.999, eps=1e-8, cost='mse_style'):
        self.alpha = alpha
        self.beta1 = beta1
        self.beta2 = beta2
        self.eps = eps
        ...
        if   cost == 'mse':
            ...
            self.cost = lambda network, x, l, y: T.mean((network(x,l) - y)**2)
    ...
    def get_cost_updates(self, network, input, output, label):
        
        cost = self.cost(network, input, label, output) + network.cost(input)
    ...
    def train(self, network, input_data, output_data, label_data, filename=None, restart=True, shuffle=True, silent=False):
        
        input = input_data.type()
        output = output_data.type()
        label = label_data.type()
        index = T.lscalar()
        ...
        cost, updates = self.get_cost_updates(network, input, output, label)
        train_func = theano.function([index], cost, updates=updates, givens={
            ...
            label:label_data[index*self.batchsize:(index+1)*self.batchsize],
