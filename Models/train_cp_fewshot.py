...
def save_network(network):
    ...
    for i in range(50):
        ...
        fnameW1 = './Parameters/' + mname + '/Fewshot/variable_dropout_time/' + network.style + ('_W1_%03i.bin' % i)        
        style_W1.astype(np.float32).tofile(fnameW1)
        
    fnameW0 = './Parameters/' + mname + '/Fewshot/variable_dropout_time/' + network.style + ('_W0.bin') 
    fnameW2 = './Parameters/' + mname + '/Fewshot/variable_dropout_time/' + network.style + ('_W2.bin') 
    fnameb = './Parameters/' + mname + '/Fewshot/variable_dropout_time/' + network.style + ('_b.bin')
...
for i, style in enumerate(styletransfer_styles):
    ...
    trainer.train(network, E, F, filename='./Parameters/' + mname + '/Fewshot/variable_dropout_time/fewshot_network.npz', restart=False, shuffle=False)
    end=time.time()
