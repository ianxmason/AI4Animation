...
class PhaseFunctionedNetwork(Layer):
    ...
    def __call__(self, input, label):
        ...
        ang_H3 = T.switch(T.all(T.eq(label[0],self.ang_label)), T.batched_dot(W2, self.dropout2(self.activation(T.batched_dot(W1, self.dropout1(H1)) + b1 + (ang_W.reshape([32,512]) * self.dropout1(H1)) + ang_b))) + b2, self.zeros)
        chi_H3 = T.switch(T.all(T.eq(label[0],self.chi_label)), T.batched_dot(W2, self.dropout2(self.activation(T.batched_dot(W1, self.dropout1(H1)) + b1 + (chi_W.reshape([32,512]) * self.dropout1(H1)) + chi_b))) + b2, self.zeros)
        dep_H3 = T.switch(T.all(T.eq(label[0],self.dep_label)), T.batched_dot(W2, self.dropout2(self.activation(T.batched_dot(W1, self.dropout1(H1)) + b1 + (dep_W.reshape([32,512]) * self.dropout1(H1)) + dep_b))) + b2, self.zeros)
        neu_H3 = T.switch(T.all(T.eq(label[0],self.neu_label)), T.batched_dot(W2, self.dropout2(self.activation(T.batched_dot(W1, self.dropout1(H1)) + b1 + (neu_W.reshape([32,512]) * self.dropout1(H1)) + neu_b))) + b2, self.zeros)
        old_H3 = T.switch(T.all(T.eq(label[0],self.old_label)), T.batched_dot(W2, self.dropout2(self.activation(T.batched_dot(W1, self.dropout1(H1)) + b1 + (old_W.reshape([32,512]) * self.dropout1(H1)) + old_b))) + b2, self.zeros)
        pro_H3 = T.switch(T.all(T.eq(label[0],self.pro_label)), T.batched_dot(W2, self.dropout2(self.activation(T.batched_dot(W1, self.dropout1(H1)) + b1 + (pro_W.reshape([32,512]) * self.dropout1(H1)) + pro_b))) + b2, self.zeros)
        sex_H3 = T.switch(T.all(T.eq(label[0],self.sex_label)), T.batched_dot(W2, self.dropout2(self.activation(T.batched_dot(W1, self.dropout1(H1)) + b1 + (sex_W.reshape([32,512]) * self.dropout1(H1)) + sex_b))) + b2, self.zeros)
        str_H3 = T.switch(T.all(T.eq(label[0],self.str_label)), T.batched_dot(W2, self.dropout2(self.activation(T.batched_dot(W1, self.dropout1(H1)) + b1 + (str_W.reshape([32,512]) * self.dropout1(H1)) + str_b))) + b2, self.zeros)
