...
Xmean = np.fromfile('./Parameters/final_diag_res/Xmean.bin', dtype=np.float32)
Ymean = np.fromfile('./Parameters/final_diag_res/Ymean.bin', dtype=np.float32)
Xstd = np.fromfile('./Parameters/final_diag_res/Xstd.bin', dtype=np.float32)
Ystd = np.fromfile('./Parameters/final_diag_res/Ystd.bin', dtype=np.float32)
...
class PhaseFunctionedNetwork(Layer):
    ...
    def __init__(self, rng=rng, input_shape=1, output_shape=1, dropout=0.7, dropout_res=0.5, style='Balance', batchsize=20):
        ...
        for i in range(4): 
            W0_load[i] = np.fromfile('./Parameters/final_diag_res/W0_%03i.bin' % (int)(i * 12.5), dtype=np.float32).reshape(512, input_shape-1)
            W1_load[i] = np.fromfile('./Parameters/final_diag_res/W1_%03i.bin' % (int)(i * 12.5), dtype=np.float32).reshape(512, 512)
            W2_load[i] = np.fromfile('./Parameters/final_diag_res/W2_%03i.bin' % (int)(i * 12.5), dtype=np.float32).reshape(output_shape, 512)

            b0_load[i] = np.fromfile('./Parameters/final_diag_res/b0_%03i.bin' % (int)(i * 12.5), dtype=np.float32)
            b1_load[i] = np.fromfile('./Parameters/final_diag_res/b1_%03i.bin' % (int)(i * 12.5), dtype=np.float32)
            b2_load[i] = np.fromfile('./Parameters/final_diag_res/b2_%03i.bin' % (int)(i * 12.5), dtype=np.float32)
    ...
    def __call__(self, input):
        ...
        style_H3 = T.batched_dot(W2, self.dropout2(self.activation(T.batched_dot(W1, self.dropout1(H1)) + b1 + (style_W0.reshape([self.batchsize,512]) * self.dropout1(H1)) + style_b))) + b2
...
def save_network(network):
    ...
    for i in range(50):
        ...
        fnameW0 = './Parameters/final_diag_res/Fewshot/1_1_dropout_time/' + network.style + ('_W0_%03i.bin' % i)        
        ...
        fnameb = './Parameters/final_diag_res/Fewshot/1_1_dropout_time/' + network.style + ('_b_%03i.bin' % i)        
...
for i, style in enumerate(styletransfer_styles):
    ...
    trainer.train(network, E, F, filename='./Parameters/final_diag_res/Fewshot/1_1_dropout_time/fewshot_network.npz', restart=False, shuffle=False)
    end=time.time()
...
outfile = open('./Training_Stats/final_diag_res_nogait_fewshot_time','wb')
